...
public class PlayerStatsControllerUI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        _xpBar.fillAmount = PlayerPrefs.GetInt(Settings.PLAYER_EXP, 0) / PlayerPrefs.GetInt(Settings.PLAYER_EXP_TO_LEVEL_UP, 100);
        _healthBar.fillAmount = PlayerPrefs.GetInt(Settings.PLAYER_HEALTH, 100) / PlayerPrefs.GetInt(Settings.PLAYER_MAX_HEALTH, 100);
        _staminaBar.fillAmount = PlayerPrefs.GetInt(Settings.PLAYER_MAX_STAMINA, 100) / PlayerPrefs.GetInt(Settings.PLAYER_STAMINA, 100);
    }
